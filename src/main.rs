...
fn hash_file(mut file_handle: File) -> io::Result<()> {
    let mut message: Vec<u8> = Vec::with_capacity(64 * MIB);
    let file_size_bytes = file_handle.read_to_end(&mut message)? as u64;
    ...
    // Add 64 bit size of non padded message.
